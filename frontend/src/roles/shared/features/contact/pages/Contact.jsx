        ...
        <div className="space-y-8">
          ...
          <Card className="space-y-4">
            ...
            <div className="space-y-3 text-sm text-slate-600 dark:text-slate-300">
              {settings?.college && (
                <p className="flex items-center gap-3"><HiMapPin className="text-primary text-lg" /> {settings.college}</p>
              )}
              {settings?.email && (
                <p className="flex items-center gap-3"><HiEnvelope className="text-accent text-lg" /> {settings.email}</p>
              )}
              {settings?.phone && (
                <p className="flex items-center gap-3"><HiUsers className="text-secondary text-lg" /> {settings.phone}</p>
              )}
            </div>
            
            {/* Social linkages */}
            <div className="flex items-center gap-4 text-xl pt-4 border-t border-slate-200/10">
              {settings?.github && <a href={settings.github} target="_blank" rel="noreferrer" className="hover:text-primary transition-colors"><SiGithub /></a>}
              {settings?.linkedIn && <a href={settings.linkedIn} target="_blank" rel="noreferrer" className="hover:text-primary transition-colors"><SiLinkedin /></a>}
              {settings?.instagram && <a href={settings.instagram} target="_blank" rel="noreferrer" className="hover:text-primary transition-colors"><SiInstagram /></a>}
            </div>
          </Card>
