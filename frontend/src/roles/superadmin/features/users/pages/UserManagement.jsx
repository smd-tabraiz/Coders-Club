...
import { HiUsers, HiMagnifyingGlass, HiShieldCheck, HiTrash, HiUserPlus, HiNoSymbol, HiDocumentText, HiDocumentArrowDown } from 'react-icons/hi2';
import Card from '../../../../../components/ui/Card';
import Badge from '../../../../../components/ui/Badge';
import { getAllUsers, toggleUserActive, changeUserRole, deleteUser } from '../../../../../features/users/user.service';
...
const UserManagement = () => {
  ...
  const toggleStatus = async (id) => {
    try {
      ...
      setUsers(prev => prev.map(u => u._id === id ? { ...u, status: res.data.data.status } : u));
    } catch (error) {
  ...
  const exportToPDF = () => {
    try {
      ...
      autoTable(doc, {
        ...
        body: filtered.map(u => [u.name || '', u.email || '', u.role || '', u.department || 'N/A', u.status === 'frozen' ? 'Frozen' : 'Active']),
        startY: 20
      });
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button onClick={exportToCSV} className="flex items-center gap-2 px-4 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold rounded-xl shadow-sm transition-all text-sm border border-slate-200 dark:border-slate-700 cursor-pointer">
            <HiDocumentArrowDown className="w-5 h-5 text-emerald-500" /> Export Excel
          </button>
          <button onClick={exportToPDF} className="flex items-center gap-2 px-4 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold rounded-xl shadow-sm transition-all text-sm border border-slate-200 dark:border-slate-700 cursor-pointer">
            <HiDocumentText className="w-5 h-5 text-rose-500" /> Export PDF
          </button>
          <a href={user?.role === 'superadmin' ? '/superadmin/add-admin' : '/admin/add-admin'}
                    ...
                    <td className="px-5 py-4 text-center">
                      {u.isSuperAdminLocked ? (
                        ...
                        <button
                          ...
                          disabled={!canEdit}
                          className={`text-2xl ${u.status === 'frozen' ? 'text-slate-300 dark:text-slate-600' : 'text-emerald-500'} hover:opacity-75 transition-opacity disabled:opacity-50`}
                          title={u.status === 'frozen' ? 'Unfreeze Account' : 'Freeze Account'}
                        >
                          {u.status === 'frozen' ? <HiNoSymbol /> : <HiShieldCheck />}
                        </button>
                      )}
                    </td>
                    <td className="px-5 py-4 text-center">
                      <button
                        ...
                        disabled={
                          u.isSuperAdminLocked ||
                          (user.role === 'admin' && (u.role === 'admin' || u.role === 'superadmin'))
                        }
