...
import { getAllEvents, deleteEvent, updateEvent } from '../../../../../features/events/event.service';
...
const AdminEvents = () => {
  ...
  const handleFileUpload = async (e, eventId) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const results = [];
        // Skip header row if it contains 'rank' or 'name'
        let startIdx = 0;
        if (json.length > 0 && typeof json[0][0] === 'string' && json[0][0].toLowerCase().includes('rank')) {
          startIdx = 1;
        }
        ...
        if (results.length === 0) {
          throw new Error("No valid data found. Ensure file has columns: Rank, Name, Dept, Score.");
        }

        // Use fetch to upload
        const token = sessionStorage.getItem('token') || '';
        const res = await fetch(`http://localhost:5000/api/events/${eventId}/results`, {
          method: 'POST',
          headers: {
        ...
        console.error(error);
        alert(error.message || 'Failed to parse/upload file');
      } finally {
        setUploadingId(null);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {canEdit && (
          <Link to={`${rolePath}/events/create`}
                ...
                >
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center justify-center gap-2">
                      <Link to={`${rolePath}/events/${ev._id}`} className="p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-500 transition-colors" title="View">
                        <HiEye className="w-4 h-4" />
                      </Link>
                      {ev.status === 'completed' && canEdit && (
                        <label className="p-2 rounded-lg hover:bg-emerald-500/10 text-emerald-500 transition-colors cursor-pointer" title="Upload CSV Results">
                          {uploadingId === ev._id ? (
                            <span className="w-4 h-4 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin inline-block"></span>
                          ) : (
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
                            </svg>
                          )}
                          <input type="file" accept=".csv, .xlsx, .xls" className="hidden" onChange={(e) => handleFileUpload(e, ev._id)} />
                        </label>
                      )}
                      {canEdit && (
                        <>
                          <Link to={`${rolePath}/events/${ev._id}/edit`} className="p-2 rounded-lg hover:bg-secondary/10 text-secondary transition-colors" title="Edit">
