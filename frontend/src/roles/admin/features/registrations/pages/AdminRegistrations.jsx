...
import { getAllRegistrations, updateRegistrationStatus } from '../../../../../features/registrations/registration.service';
...
const AdminRegistrations = () => {
  ...
  const events = ['all', ...new Set(registrations.map(r => r.eventId?.name || r.eventId?.title || r.event))];

  const filtered = registrations.filter(r => {
    const studentName = r.name || r.student || r.userId?.name || '';
    const studentRoll = r.rollNo || r.userId?.rollNo || '';
    const eventName = r.eventId?.name || r.eventId?.title || r.event || '';
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Student Name', 'Email', 'Phone', 'Roll No', 'Event', 'Department', 'Year', 'Status', 'Registration Date', 'Team Role'];
    
    const csvContent = [
      headers.join(','),
      ...filtered.flatMap(r => {
        const eventName = (r.eventId?.name || r.eventId?.title || r.event || '').replace(/,/g, '');
        const status = r.status || '';
        const date = new Date(r.createdAt || r.registeredAt).toLocaleDateString();
        const teamName = (r.teamName || '').replace(/,/g, '');
        const isTeam = !!teamName || (r.teamMembers && r.teamMembers.length > 0);

        // Primary Registrant
        const primary = [
          (r.name || r.student || r.userId?.name || '').replace(/,/g, ''),
          (r.email || r.userId?.email || '').replace(/,/g, ''),
          (r.phone || r.userId?.phone || '').replace(/,/g, ''),
          (r.rollNo || r.userId?.rollNo || '').replace(/,/g, ''),
          eventName,
          (r.branch || r.userId?.department || '').replace(/,/g, ''),
          r.year || r.userId?.year || '',
          status,
          date,
          isTeam ? `Team Lead: ${teamName}` : 'Individual'
        ].join(',');

        // Team members
        const members = (r.teamMembers || []).map(m => {
          return [
            (m.name || '').replace(/,/g, ''),
            (m.email || '').replace(/,/g, ''),
            (m.phone || '').replace(/,/g, ''),
            ...
            status,
            date,
            isTeam ? `Team Member: ${teamName}` : 'Member'
          ].join(',');
        });
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
        ...
        <Button variant="outline" onClick={handleExportCSV} className="whitespace-nowrap flex items-center gap-2">
                ...
                >
                  <td className="px-5 py-4 min-w-[200px]">
                    <p className="font-semibold text-slate-800 dark:text-white">
                      {r.teamName ? `Team: ${r.teamName}` : (r.name || r.userId?.name)}
                    </p>
                    {!r.teamName && (
                    ...
                    {r.teamName && (
                      <div className="mt-2 text-[11px] text-slate-600 dark:text-slate-400 space-y-1 border-t border-slate-200/50 dark:border-slate-700/50 pt-2">
                        <div className="mb-2 space-y-0.5">
                          <p className="font-semibold text-[10px] uppercase text-primary">Lead: {r.name || r.userId?.name}</p>
                          <p className="text-[10px] text-slate-500">Email: {r.email || r.userId?.email}</p>
                          <p className="text-[10px] text-slate-500">Phone: {r.phone || r.userId?.phone || 'N/A'}</p>
                        </div>
                        {r.teamMembers?.map((m, i) => (
                          <p key={i} className="flex gap-2 justify-between">
                            <span>• {m.name}</span>
                            <span className="text-slate-400 font-mono">{m.rollNo}</span>
                          </p>
                        ))}
                      </div>
                    )}
                  ...
                  <td className="px-5 py-4 text-slate-600 dark:text-slate-300 hidden md:table-cell text-xs">{r.eventId?.name || r.eventId?.title || r.event}</td>
