...
import { addAdmin } from '../../api/user.service';

const AddAdmin = () => {
  const { user: currentUser } = useAuth();
  const isSuperAdmin = currentUser?.role === 'superadmin';

  const [form, setForm] = useState({ name: '', email: '', department: 'Faculty', password: '', confirmPassword: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(true);
      setForm({ name: '', email: '', department: 'Faculty', password: '', confirmPassword: '' });
    } catch (err) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <select name="department" value={form.department} onChange={handleChange} className={inputCls}>
                {['Faculty', 'CSE', 'IT', 'ECE', 'EEE', 'MECH', 'CIVIL', 'Management'].map(d => (
