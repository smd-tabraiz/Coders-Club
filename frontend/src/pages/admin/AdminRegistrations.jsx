...
import { getAllRegistrations, updateRegistrationStatus } from '../../api/registration.service';
    ...
    <div className="space-y-8">
      ...
      <div>
        <h1 className="text-3xl font-extrabold text-slate-800 dark:text-white flex items-center gap-3">
          <HiClipboardDocumentList className="text-primary" /> {canEdit ? 'Registrations' : 'All Registrations (View Only)'}
        </h1>
        <p className="text-sm text-slate-500 mt-1">
          {canEdit ? 'Review, approve or reject student event registrations.' : 'You have read-only access to all registration records.'}
        </p>
      </div>
      {!canEdit && (
        <div className="flex items-center gap-3 p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-xs text-emerald-600 dark:text-emerald-400 font-semibold">
          👁️ Club Member — Read-Only View. Approve/Reject actions are only available to admins.
        </div>
      )}
