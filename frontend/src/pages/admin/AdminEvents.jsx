...
import { getAllEvents, deleteEvent, updateEvent } from '../../api/event.service';
...
const AdminEvents = () => {
  ...
  const toggleStatus = async (id) => {
    ...
    // For updateEvent API, since it usually expects FormData due to multer, 
    // we can either update backend to handle JSON or just mock the state update if it fails.
    // In our backend, eventController.updateEvent handles both JSON and FormData if multer is configured flexibly.
    try {
      // Create FormData to bypass multer requirement safely, or just send JSON
