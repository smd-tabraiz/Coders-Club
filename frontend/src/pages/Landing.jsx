    ...
    <div id="home" className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300 overflow-x-hidden selection:bg-primary/20 selection:text-primary">
      ...
      <section id="events" className="py-24 max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            { title: 'Code Sprint 2026', desc: 'Annual algorithm optimization challenge with certificates and prizes worth ₹50,000.', date: 'Oct 15, 2026', venue: 'CSE Seminar Hall' },
            { title: 'React Web Bootcamp', desc: 'A 3-day deep dive into modern frontend frameworks, Tailwind, and fullstack integrations.', date: 'Nov 05, 2026', venue: 'Computer Lab 3' },
            { title: 'Hackathon 5.0', desc: 'GPREC’s premier coding hackathon - build solutions for local college administration.', date: 'Dec 12, 2026', venue: 'Auditorium' }
          ].map((evt, idx) => (
            <div key={idx} className="glass rounded-2xl overflow-hidden border border-slate-200/20 shadow-lg hover:shadow-xl transition-all flex flex-col justify-between h-full">
              ...
              <div className="p-6 space-y-4 flex-1 flex flex-col justify-between">
                <div>
                  <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">{evt.title}</h3>
                  <p className="text-slate-500 text-sm mb-4 leading-relaxed">{evt.desc}</p>
                </div>
                <div className="space-y-3">
                  <div className="flex justify-between text-xs font-semibold text-primary-500 dark:text-primary-400">
                    <span>📅 {evt.date}</span>
                    <span>📍 {evt.venue}</span>
                  </div>
                  <Link
      ...
      <section id="gallery" className="py-24 max-w-7xl mx-auto px-6 border-t border-slate-200/10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 auto-rows-[200px]">
          {[
            ...
            'from-primary-600 to-slate-900',
            'from-emerald-500 to-cyan-500',
            'from-rose-500 to-orange-500'
          ].map((grad, idx) => (
            <div
              key={idx}
              className={`rounded-2xl bg-gradient-to-tr ${grad} relative overflow-hidden group shadow-md hover:scale-[1.02] transition-all duration-300 ${
                idx === 1 ? 'row-span-2' : ''
              }`}
            >
